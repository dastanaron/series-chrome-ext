...
function renderResult(data: SeriesInfo[]) {
  ...
  data.forEach((value, index) => {
    html += `<tr><td>${index + 1}</td><td>${value.name}</td><td>${value.season}</td><td>${value.episode}</td><td><a href="${value.url}" target="_blank">Go to...</a></td><td><button class="delete" data-id="${value.id}" type="button"><svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
